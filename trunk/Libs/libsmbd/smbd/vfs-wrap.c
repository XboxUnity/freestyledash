...
#ifdef _XBOX
	if (strcmp(fname, ".") == 0) {
		char* path = smb_getcwd(NULL, 1024);
		result = sys_opendir(path);
		free(path);
	}
		...
		if (strstr(fname, ":")) {
			///see if it's already got a : in it
			result = sys_opendir(fname);
		}
		else {
			char* fullname = smb_getcwd(NULL, 1024);
			char* p;
			strncat(fullname,"\\",1024);
			strncat(fullname, fname, 1024);
...
int vfswrap_rename(vfs_handle_struct *handle, connection_struct *conn, const char *oldname, const char *newname)
{
	...
	if (strstr(oldname, ":")) {
		...
		old = smb_getcwd(NULL, 1024);
		strncat(old,"\\",1);

		if (strncmp(oldname, "./", 2) == 0) {
			strncat(old, oldname+2, 1024);
		}
		else {
			strncat(old, oldname, 1024);
		}
	...
	if (strstr(newname, ":")) {
		...
		newn = smb_getcwd(NULL, 1024);
		strncat(newn,"\\",1);

		if (strncmp(newname, "./", 2) == 0) {
			strncat(newn, newname+2, 1024);
		}
		else {
			strncat(newn, newname, 1024);
		}
	...
	result = rename(old, newn);
	if (errno == EXDEV) {
		/* Rename across filesystems needed. */
		result = copy_reg(old, newn);
	}
...
int vfswrap_stat(vfs_handle_struct *handle, connection_struct *conn, const char *fname, SMB_STRUCT_STAT *sbuf)
{
	...
	if (strstr(fname, ":")) {
		...
		result = sys_stat(temp, sbuf);
		free(temp);
	}
	else {
		fullname = smb_getcwd(NULL, 1024);
		if (strncmp(fname, "./", 2) == 0) {
			strncat(fullname, "\\", 1);
			strncat(fullname, fname+2, 1024);
		}
		else if (strncmp(fname, ".", 1) ==0 ) {
			;  // do nothing
		}
		...
		{
			char* pos = fullname+strlen(fullname)-3;
			if (strncmp(pos, ":\\.", 3) == 0) {
				*(pos+1) = 0;
			}
		}
...
int vfswrap_lstat(vfs_handle_struct *handle, connection_struct *conn, const char *path, SMB_STRUCT_STAT *sbuf)
{
	...
	if (strstr(path, ":")) {
		...
		result = sys_lstat(fullname, sbuf);
		free(fullname);
	}
