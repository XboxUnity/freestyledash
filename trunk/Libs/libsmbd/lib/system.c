...
static pdevices_list DevicesList;

pdevices_list getDeviceId(const char *device) 
{
	pdevices_list devList = DevicesList;
	pdevices_list last;
	int id;

	if (DevicesList == NULL) {
		DevicesList = (pdevices_list)malloc(sizeof(devices_list));
		DevicesList->device = strdup(device);
		DevicesList->dev_id = 1;
		DevicesList->files = NULL;
		DevicesList->next = NULL;
		return DevicesList;
	}

	while (devList) {
		if (strncmp(devList->device, device, 25) == 0) 
			return devList;
		last = devList;
		id = devList->dev_id;
	...
	last->next = (pdevices_list)malloc(sizeof(devices_list));
	last->next->device = strdup(device);
	last->next->dev_id = id + 1;
	last->next->files = NULL;
	last->next->next = NULL;
	return last->next;
}

int getFileId(pdevices_list dev, const char*fname) 
{
	pfiles f = dev->files;
	pfiles last;
	int id;
	...
	if (dev->files == NULL) {
		dev->files = (pfiles)malloc(sizeof(files));
		dev->files->filename = strdup(fname);
		dev->files->file_id = 1;
		dev->files->next = NULL;
		return 1;
	}
	...
	while (f) {
		if (strncmp(f->filename, fname, MAX_PATH) == 0)
			return f->file_id;
		last = f;
		id = f->file_id;
		f = f->next;
	...
	last->next = (pfiles)malloc(sizeof(files));
	last->next->filename = strdup(fname);
	last->next->file_id = id + 1;
	last->next->next = NULL;
	return id + 1;
}

void getUniqueInodeAndDevice(const char *fname, SMB_STRUCT_STAT *sbuf)
{
	char* offset;
	char device[15];
	char filename[MAX_PATH];
	pdevices_list devEntry;

	offset = strstr(fname, ":");
	strncpy(device, fname, offset-fname);
	device[offset-fname] = 0;
	devEntry = getDeviceId(device);
	sbuf->st_dev = devEntry->dev_id;
	strncpy(filename, offset+1, MAX_PATH);
	sbuf->st_ino = getFileId(devEntry, filename);
}
#endif
...
int sys_stat(const char *fname,SMB_STRUCT_STAT *sbuf)
{
	...
	if (fname[len -1] == ':') {
		// Todo...make sure this is actually mounted
		ret = 0;  
		sbuf->st_mode = 0;
	...
	if (ret == 0)
		getUniqueInodeAndDevice(fname, sbuf);
#endif
#else
	ret = stat(fname, sbuf);
#endif
...
int sys_fstat(int fd,SMB_STRUCT_STAT *buf)
{
	int ret;
...
//		h = (HANDLE)_osfhnd(fd);
		...
		if (GetFileInformationByHandle(fd, &bhfi) == 0) {
			_dosmaperr(GetLastError());
		    return -1;
		}
	}
	if (fd < 0 && bhfi.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
             ...
             bhfi.ftLastWriteTime.dwHighDateTime )
        {
            ...
            buf->st_mtime = __loctotime64_t(SystemTime.year,
                                            SystemTime.month,
                                            SystemTime.day,
                                            SystemTime.hour,
                                            SystemTime.minute,
             ...
             bhfi.ftLastAccessTime.dwHighDateTime )
        {
            ...
            buf->st_atime = __loctotime64_t(SystemTime.year,
                                          SystemTime.month,
                                          SystemTime.day,
                                          SystemTime.hour,
                                          SystemTime.minute,
             ...
             bhfi.ftCreationTime.dwHighDateTime )
        {
            ...
            buf->st_ctime = __loctotime64_t(SystemTime.year,
                                          SystemTime.month,
                                          SystemTime.day,
                                          SystemTime.hour,
                                          SystemTime.minute,
...
#endif
	/* we always want directories to appear zero size */
	if (ret == 0 && S_ISDIR(buf->st_mode)) buf->st_size = 0;
	return ret;
}
...
int sys_open(const char *path, int oflag, mode_t mode)
{
	...
	return open64(path, oflag, mode);
		...
		if (ret == -1 && oflag == _O_BINARY && mode == 0x1e4) {
			DEBUG(1,("trying to open as a dir here [%s]\n", path));
			ret = try_open_as_dir(path);			
			DEBUG(1,("sys_open returning %08x for %s\n", ret, path));
			return ret;
		}
		else {
//			DEBUG(1,("sys_open returning %08x for %s\n", ret, path));
