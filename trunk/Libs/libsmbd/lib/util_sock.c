...
static void print_socket_options(int s)
{
	...
	for (; p->name != NULL; p++) {
#ifdef _XBOX
		if (p->option == SO_KEEPALIVE || p->option == SO_BROADCAST)
			continue;
#endif
...
int open_socket_in( int type, int port, int dlevel, uint32 socket_addr, BOOL rebind )
{
	...
	/* This block sets/clears the SO_REUSEADDR and possibly SO_REUSEPORT. */
	{
  	...
  	if(setsockopt(res, SOL_SOCKET, 0x5802, (PCSTR)&bBroadcast, sizeof(BOOL) ) != 0 )//PATCHED!
	{
//		DebugMsg("Debug","Failed to set debug recieve socket to 5802, error");
		return -1;
	}

	if(setsockopt(res, SOL_SOCKET, 0x5801, (PCSTR)&bBroadcast, sizeof(BOOL) ) != 0 )//PATCHED!
	{
