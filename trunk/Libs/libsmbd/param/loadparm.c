...
BOOL bLoaded = False;
	...
	void xb_setSambaWorkgroup(const char* workgroup)
	{
		SAFE_FREE(smbWORKGROUP);
		smbWORKGROUP = strdup(workgroup);
	}

	char* smbHOSTNAME = NULL;

	void xb_setSambaHostname(const char* hostname)
	{
		SAFE_FREE(smbHOSTNAME);
		smbHOSTNAME = strdup(hostname);
	}

	static int g_SambaClientCount = 0;
	void xb_IncClientCount() {
		g_SambaClientCount++;
		DEBUG(0,("xb_IncClientCount: Client count is now: %d\n", g_SambaClientCount));
	}
	void xb_DecClientCount(){
		g_SambaClientCount--;
		DEBUG(0,("xb_DecClientCount: Client count is now: %d\n", g_SambaClientCount));
	}

	int getSambaClientCount() {
		return g_SambaClientCount;
	}
#endif
