...
static int socket_wait(netbuf *ctl)
{
	...
	do
	{
		...
		{
			rv = 0;
			strncpy_s(ctl->ctrl->response, 255, strerror(errno), sizeof(ctl->ctrl->response));
			break;
		}
		else if (rv > 0)
		{
...
static int readline(char *buf,int max,netbuf *ctl)
{
	...
	if ((ctl->dir != FTPLIB_CONTROL) && (ctl->dir != FTPLIB_READ)) {
		DebugMsg("FTPLib", "readline failed?");
		return -1;
	}
	if (max == 0)
		return 0;
	do
	{
		if (ctl->cavail > 0)
		{
			...
			*bp = '\0';
			...
			if (end != NULL)
			{
				...
				if (retval == -1) {
					DebugMsg("FTPLib", "readline failed in processing?");
				}
				break;
			}
		}
		...
		{
			if (retval == 0) {
				DebugMsg("FTPLib", "readline failed -eof");
				retval = -1;
			}
			break;
		}
		if (!socket_wait(ctl)){
			if (retval == -1) {
				DebugMsg("FTPLib", "readline socket_wait failed");
			}
			return retval;
		}
		if ((x = net_read(ctl->handle,ctl->cput,ctl->cleft)) == -1)
		{
			...
			DebugMsg("FTPLib", "readline net_read(recv) failed %d", err);
			perror("read");
			retval = -1;
			break;
		}
...
static int readresp(char c, netbuf *nControl)
{
	...
	{
		DebugMsg("FTPLib", "Control socket read failed");
		perror("Control socket read failed");
		return 0;
	}
//	DebugMsg("readresp [%s]", nControl->response);
	if (ftplib_debug > 1)
	...
	if (nControl->response[3] == '-')
	{
		...
		do
		{
			if (readline(nControl->response,256,nControl) == -1)
			{
				DebugMsg("FTPLib", "B Control socket read failed");
				perror("Control socket read failed");
				return 0;
			}
			if (ftplib_debug > 1)
...
static int readCustomResp(char* resp, int length, netbuf *nControl)
{
	int len;
//	DebugMsg("about to call readline");
	if (readline(nControl->response,256,nControl) == -1)
	{
		DebugMsg("FTPLib", "readCustomResp Control socket read failed");
		return 0;
	}
	len = strlen(nControl->response);
//	DebugMsg("read a total of %d chars", len);
	if (ftplib_debug > 1)
		fprintf(stderr,"%s",nControl->response);

	int rl = len < length-1 ? len : length-1;
	strncpy(resp, nControl->response, rl);
	resp[rl+1] = 0;
//	DebugMsg("readCustomResp read %s", resp);
	return 1;
}
...
GLOBALDEF int FtpConnect(const char *host, int port, netbuf **nControl)
{
	...
	if (sControl == -1)
	{
		DebugMsg("FTPLib", "FtpConnect about to return 0.  Couldn't create socket [%ld]", WSAGetLastError());
		perror("socket");
		return 0;
	}
	free(lhost);
...
#ifdef _XBOX
//	patchSocket(sControl);

	BOOL bBroadcast = TRUE;
/*	if( setsockopt(sControl, SOL_SOCKET, 0x5802, (PCSTR)&bBroadcast, sizeof(BOOL) ) != 0 )//PATCHED!
	{
		DebugMsg( "FTPLib","Failed to set socket to 5802, error");
		return -1;
	}
*/
	if( setsockopt(sControl, SOL_SOCKET, 0x5801, (PCSTR)&bBroadcast, sizeof(BOOL) ) != 0 )//PATCHED!
	{
		DebugMsg( "FTPLib","Failed to set socket to 5801, error");
		return -1;
	}
	...
	{
		DebugMsg("FTPLib", "FtpConnect about to return 0.  Couldn't setsockopt [%ld]", WSAGetLastError());
		perror("setsockopt");
		net_close(sControl);
		return 0;
	}
	...
	{
		DebugMsg("FTPLib", "FtpConnect about to return 0.  Couldn't connect [%ld]", WSAGetLastError());
		perror("connect");
		net_close(sControl);
		return 0;
	}
	...
	{
		DebugMsg("FTPLib", "FtpConnect readresp returned 0");
		net_close(sControl);
		free(ctrl->buf);
		free(ctrl);
		return 0;
	...
	*nControl = ctrl;
	DebugMsg("FTPLib", "FtpConnect setting conn to %ld, socket is %d", ctrl, ctrl->handle);
	return 1;
}

/*
...
static int FtpSendCmd(const char *cmd, char expresp, netbuf *nControl)
{
	...
	if (ftplib_debug > 2)
		fprintf(stderr,"%s\n",cmd);
//	DebugMsg("FtpSendCmd Sending [%s]", cmd);
	if ((strlen(cmd) + 3) > sizeof(buf))
		return 0;
	sprintf(buf,"%s\r\n",cmd);
	if (net_write(nControl->handle,buf,strlen(buf)) <= 0)
	{
		DebugMsg("FTPLib", "FtpSendCmd net_write finished <=0");
		//perror("write");
		return 0;
	}
	int r = readresp(expresp, nControl); 
	if (r == 0) {
		DebugMsg("FTPLib", "FtpSendCmd readresp == 0");
	}
	return r;
}

static int FtpSendCustomCmd(const char *cmd, char *resp, int length, netbuf *nControl)
{
	char buf[256];
	if (nControl->dir != FTPLIB_CONTROL)
		return 0;
	if (ftplib_debug > 2)
		fprintf(stderr,"%s\n",cmd);
//	DebugMsg("FtpSendCustomCmd Sending [%s]", cmd);
	if ((strlen(cmd) + 3) > sizeof(buf))
		return 0;
	sprintf(buf,"%s\r\n",cmd);
	...
	if (net_write(nControl->handle,buf,strlen(buf)) <= 0)
	{
		DebugMsg("FTPLib", "net_write finished <=0");
		//perror("write");
		return 0;
	}
//	DebugMsg("net_write finished");
	return readCustomResp(resp, length, nControl);
}
...
//	patchSocket(sData);
	...
	BOOL bBroadcast = TRUE;
/*	if( setsockopt(sData, SOL_SOCKET, 0x5802, (PCSTR)&bBroadcast, sizeof(BOOL) ) != 0 )//PATCHED!
	{
		DebugMsg( "FTPLib","Failed to set socket to 5802, error");
		return -1;
	}
*/
	if( setsockopt(sData, SOL_SOCKET, 0x5801, (PCSTR)&bBroadcast, sizeof(BOOL) ) != 0 )//PATCHED!
	{
		DebugMsg( "FTPLib","Failed to set socket to 5801, error");
		return -1;
	}
...
static int FtpAcceptConnection(netbuf *nData, netbuf *nControl)
{
	...
	else
	{
		...
		else if (FD_ISSET(nControl->handle, &mask))
		{
			net_close(nData->handle);
			nData->handle = 0;
			int r = readresp('2', nControl);
			if (r == 0) {
				DebugMsg("FTPLib", "readresp ret 0 in FtpAcceptConnection");
			}
			rv = 0;
		}
	}
...
//	DebugMsg("In FtpAccess Before call to FtpSendCmd");
	if (!FtpSendCmd(buf, '2', nControl))
		return 0;
//	DebugMsg("After call to FtpSendCmd");
	switch (typ)
	{
	case FTPLIB_DIR:
		strcpy(buf,"NLST");
	...
	if (path != NULL)
	{
		...
		if ((strlen(path) + i) >= sizeof(buf)) {
			DebugMsg("FTPLib", "Bailing due to path?");
			return 0;
		}
		strcpy(&buf[i],path);
	}
//	DebugMsg("Before call to FtpOpenPort");
	if (FtpOpenPort(nControl, nData, mode, dir) == -1)
		return 0;
//	DebugMsg("Before call to FtpSendCmd");
...
//		DebugMsg("FtpWrite tried to write %d, returned %d", len, i);
	}
	if (i == SOCKET_ERROR) {
		DWORD dw = GetLastError();
		DebugMsg("FTPLib", "GetLastError was %08x", dw);
	}
	if (i == -1) 
		return 0;
	nData->xfered += i;
...
GLOBALDEF int FtpClose(netbuf *nData)
{
	...
	case FTPLIB_READ:
		if (nData->buf)
			free(nData->buf);
		r = shutdown(nData->handle,2);

		s = net_close(nData->handle);
//		DebugMsg("Shutdown returned %d and close return %d for %ld", r, s, nData->handle);
		ctrl = nData->ctrl;
		free(nData);
		if (ctrl)
		{
			ctrl->data = NULL;
			int r = readresp('2', ctrl); 
			if (r == 0) {
				DebugMsg("FTPLib", "FtpClose readresp == 0");
			}
			return r;
		}
		return 1;
				   ...
				   netbuf *nControl, int typ, int mode)
{
	...
	if (localfile != NULL)
	{
		...
		if (local == NULL)
		{
			...
			DebugMsg("FtpXfer Error: %s", nControl->response);
			return 0;
		}
/*		fseek(local, 0, SEEK_END);
		int pos = ftell(local);
		fseek(local, 0, SEEK_SET);
		DebugMsg("File size is %d", pos);*/
	}
	if (local == NULL)
		local = (typ == FTPLIB_FILE_WRITE) ? stdin : stdout;
//	DebugMsg("In FtpXfer, about to call Access");
	...
	if (typ == FTPLIB_FILE_WRITE)
	{
		...
		DebugMsg("read %d %d %d", accum, feof(local), ferror(local));

		fseek(local, 0, SEEK_SET);
*/

		bool failed = false;
		DebugMsg("FTPLib", "In FtpXfer, about the enter while loop to read");
		while ((l = fread(dbuf, 1, FTPLIB_BUFSIZ, local)) > 0 && !failed) {
			int tosend = l;
			int sent = 0;
			int offset = 0;
			while (tosend > 0)
			{
				...
				sent = FtpWrite(dbuf+offset, tosend, nData);
		...
		DebugMsg("FTPLib", "wrote %d %d %d", nData->xfered, feof(local), ferror(local));
	}
	else
	{
...
//MFMT 20100429204347 XM360.xex
GLOBALDEF int FtpSetModDate(const char *path, FILETIME ft, netbuf *nControl)
{
	char buf[256];
	int rv = 1;

	if ((strlen(path) + 20) > sizeof(buf))
		return 0;

	SYSTEMTIME st;
	FileTimeToSystemTime(&ft, &st);
	sprintf_s(buf, "MFMT %04u%02u%02u%02u%02u%02u %s", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, path);
	if (!FtpSendCmd(buf,'2',nControl))
		rv = 0;
	return rv;
}
...
/*
* FtpCustom - send a custom command
*
* return 1 if successful, 0 otherwise
*/
GLOBALDEF int FtpCustom(const char *cmd, char* resp, int length, netbuf *nControl)
{
	return FtpSendCustomCmd(cmd, resp, length, nControl);
}
