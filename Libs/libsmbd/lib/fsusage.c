...
int sys_fsusage(const char *path, SMB_BIG_UINT *dfree, SMB_BIG_UINT *dsize)
{
	...
	if (!strstr(path, ":")) 
		cwd = smb_getcwd(NULL, 1024);
	else 
		cwd = strdup(path);  // since it's const
	...
	GetDiskFreeSpaceEx(c,&m_FreeBytesAvailable,&m_TotalNumberOfBytes,&m_TotalNumberOfFreeBytes);

	*dfree = m_FreeBytesAvailable.QuadPart/512;
	*dsize = m_TotalNumberOfBytes.QuadPart/512;
